...
mydb = mysql.connector.connect(
  host='localhost',
  user='root',
  password='skpurple@123',
  database = 'organ_transplant'
)
...
def stats():
    ...
    plt.title('Donor Organ Distribution')
    plt.savefig(static_dir + 'donor_stat.jpeg')
    plt.close()
    ...
    plt.pie(B, labels=A, autopct='%1.1f%%')
    plt.title('Patient Organ Requests')
    plt.savefig(static_dir + 'Patient_stat.jpeg')
    plt.close()

    # Success vs Failure in Transplantation
    qry = "SELECT DISTINCT Organ_donated FROM Transaction INNER JOIN Donor ON Transaction.Donor_ID = Donor.Donor_ID"
    mycursor.execute(qry)
    list = mycursor.fetchall()
    organ_list = [organ[0] for organ in list]
    ...
    for organ in organ_list:
        ...
        mycursor.execute(qry)
        a = mycursor.fetchone()
        A.append(a[0] if a[0] is not None else 0)
        ...
        mycursor.execute(qry)
        b = mycursor.fetchone()
        B.append(b[0] if b[0] is not None else 0)
    ...
    plt.ylabel('Number of Transplantations')
    plt.xlabel('Organ')
    plt.title('Success vs Failure in Organ Transplantation')
    plt.xticks(ind + width / 2, organ_list)
    plt.legend(loc='best')
    plt.savefig(static_dir + 'success.jpeg')
    plt.close()
